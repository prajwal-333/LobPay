...
create_merchant_table = """CREATE TABLE IF NOT EXISTS Merchants(id INTEGER PRIMARY KEY NOT NULL, address text, account_info text,pin INTEGER,lat REAL, long REAL, subscribers INTEGER NOT NULL, FOREIGN KEY(id) REFERENCES Users(id) ON DELETE CASCADE);"""
...
create_invoice_table = """CREATE TABLE IF NOT EXISTS Invoice(id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, cid INTEGER NOT NULL, inv_date DATE,total_price REAL,FOREIGN KEY(cid) REFERENCES Customers(id) ON DELETE CASCADE)"""
create_inventory_merchant_table = """CREATE TABLE IF NOT EXISTS Merchants_Inventory(id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, mid INTEGER NOT NULL, quantity INTEGER NOT NULL,price REAL, item_id INTEGER NOT NULL,FOREIGN KEY(mid) REFERENCES Merchants(id) ON DELETE CASCADE,FOREIGN KEY(item_id) REFERENCES Inventory(id) ON DELETE CASCADE)"""
...
c.execute("""DROP TABLE IF EXISTS Merchants_Inventory;""")
...
c.execute(create_inventory_merchant_table)
